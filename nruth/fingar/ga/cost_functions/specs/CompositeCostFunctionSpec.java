...
import nruth.fingar.ga.cost_functions.HeijinkPenaliseHighNeckPositionCostFunction;
...
public class CompositeCostFunctionSpec {
	...
	public void coefficients_available_for_feature_weights(){
		new CompositeCostFunction(1, 1, 1);
	}
	...
	public void weightings_affect_costs_correctly(){
		...
		assertEquals(new CompositeCostFunction(1, 0, 0).determine_cost(arr1),
		new HeijinkPenaliseHighNeckPositionCostFunction().determine_cost(arr2));
		
		assertEquals(new CompositeCostFunction(0, 1, 0).determine_cost(arr1),
		new StringChangeCostFunction().determine_cost(arr2));
		
		assertEquals(new CompositeCostFunction(0, 0, 1).determine_cost(arr1),
		new SimpleHandPositionModelCostFunction().determine_cost(arr2));
		
		assertEquals(new CompositeCostFunction(0, 0, 2).determine_cost(arr1),
				new SimpleHandPositionModelCostFunction().determine_cost(arr2)*2);
	}
}
